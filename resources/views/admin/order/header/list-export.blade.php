                                ...
                                <div class="col-sm-2"  style="float:left; margin-top:18px;">
                                    ...
                                    <div class="form-group">
                                        <div class="form-group input-from">
                                            <label style="margin-bottom: 15px;">WIP</label>
                                            <!--<input type="text"  class="form-control" id="WIP" name="WIP" placeholder="WIP" required onKeyup="searchInputFilterWIP()">-->
                                            <input type="text"  class="form-control" id="WIP_search" name="WIP_search" placeholder="WIP" required>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-3"  style="float:left; margin-top:20px;">
                                    <!-- checkbox -->
...
<<<<<<< HEAD
                                <div class="col-sm-7" style="float:right">
                                    ...
                                    <div class="form-group date-form">
                                        <label class="form-control-label" style="width: 25px;">To</label>
                                        <div class="form-control-panel">
...
<<<<<<< HEAD
                        				 	        @if($exp[1] == 'Installation Time' && !empty( $settingTableInfo) && $settingTableInfo->status == 1)
                        				 	            <td style="background-color:#E8ECF1;" class="INSTALLATION_TIME_click" id="{{ $salesOrderInfo->ID }}">
                                            				...
                                            				<span id="INSTALLATION_TIME_{{ $salesOrderInfo->ID }}" class="textStatus" style="text-align: center;display: block;">{{ $salesOrderInfo->INSTALLATION_TIME }}</span>
                        				 	        ...
                        				 	        @if($exp[1] == 'Designer Email Address' && !empty( $settingTableInfo) && $settingTableInfo->status == 1)
                                                    	<td style="background-color:#E8ECF1;" class="DESIGNER_EMAIL_ADDRESS_click" id="{{ $salesOrderInfo->ID }}">
                                    						<span id="DESIGNER_EMAIL_ADDRESS_{{ $salesOrderInfo->ID }}" class="textStatus">{{ $salesOrderInfo->DESIGNER_EMAIL_ADDRESS }}</span>
...
<<<<<<< HEAD
 ...
 $(document).on('keyup click change', '.DESIGNER_EMAIL_ADDRESS_click', function() {
    ...
    var ID    = $(this).attr('id');
    var first = $("DESIGNER_EMAIL_ADDRESS_input_"+ID).val();
    ...
    $.ajax({
        ...
        data: {
            '_token': $('input[name=_token]').val(),
            'id': $(this).attr('id'),
            'DESIGNER_EMAIL_ADDRESS': $("#DESIGNER_EMAIL_ADDRESS_input_"+ID).val(),
            'type':12
        },
        url: baseUrl +'/sales_update' , 
        success: function(html) {
                $("#DESIGNER_EMAIL_ADDRESS_"+ID).html(html);
            }
        });

}).change(function() {});

$(document).on('keyup click change', '.INSTALLATION_TIME_click', function() {
    ...
    var ID    = $(this).attr('id');
    var first = $("#INSTALLATION_TIME_input_"+ID).val();
    ...
    $.ajax({
        ...
        data: {
            '_token': $('input[name=_token]').val(),
            'id': $(this).attr('id'),
            'INSTALLATION_TIME': $("#INSTALLATION_TIME_input_"+ID).val(),
            'type':13
        },
        url: baseUrl +'/sales_update' , 
        success: function(html) {
            $("#INSTALLATION_TIME_"+ID).html(html);
            }
        });
